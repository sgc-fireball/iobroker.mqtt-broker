...
class MqttServer {
    ...
    constructor(options, adapter) {
        ...
        this.options = {
            ...
            onPublish: (/*client, topic, value*/) => {
                this.log('INVALID onPublish CALLLBACK');
                // do nothing
            }
        }
    ...
    _onConnect(client, packet) {
        ...
        this.options.checkCredentials(username, password)
            .then(() => {
                ...
                this.log('Connection accepted. Client: '+client._id+' / '+client._username);
    ...
    _onPublish(client, packet) {
        client._lastSeen = Date.now();
        this.log('_onPublish: '+JSON.stringify(packet));
        ...
        this.log('Receieved message from '+client._id+' in '+topic);
    ...
    _onPubRel(client, packet) {
        ...
        if (client._incoming.hasOwnProperty(packet.messageId)) {
            ...
            this._onPublish(client, {...packet, qos: 0});
        }
    }
