...
class MqttServer {
    ...
    _onConnect(client, packet) {
        ...
        this.options.checkCredentials(username, password)
            ...
            .catch((e) => {
                this.log('Reject client connection: '+e.toString());
    ...
    async _onSubscribe(client, packet) {
        client._lastSeen = Date.now();
        let granted = [];
        this.log('Client '+client._id+': '+JSON.stringify(packet.subscriptions));
        for (let i = 0; i < packet.subscriptions.length; i++) {
            const topic = packet.subscriptions[i].topic;
            const qos = packet.subscriptions[i].qos;

            this.log('Client '+client._id+': check access to topic '+topic+' with qos level '+qos);
            let result = 128;
            try {
                if (await this.options.canSubscribe(client, topic, qos)) {
                    this.log('Client '+client._id+': grant subscription: '+topic+' with qos level '+qos);
                    result = qos;
                ...
                this.log('Client '+client._id+': reject subscription: '+topic+' with qos level '+qos);
                this.log('Client '+client._id+': '+e.toString());
            }
            granted.push(qos);
        }
        this.log('Client '+client._id+': '+JSON.stringify(granted));
    ...
    _onClose(client, error = null) {
        ...
        error = error || 'unknown';
        this.log('Client ' + client._id + ' connection closed: ' + error.toString());
