...
class Adapter extends utils.Adapter {
    ...
    _preloadingStates() {
        this.getForeignStates('*', (err, res) => {
            if (!err && res) {
                ...
                this.log.info('Preloading ' + keys.length + ' states.');
                return;
            }
    ...
    _onClientPublish(client, topic, value) {
        if (/^\$/.test(topic) || /^system/.test(topic)) {
            this.log.info(client._id + ': skip illegal topic ' + topic);
            return;
        }
        if (/^rpc\//.test(topic)) {
        ...
        if (!this.states.hasOwnProperty(id)) {
            this.log.info(client._id + ': try to set an unknown id ' + id);
            return;
        }
        this.log.info(client._id + ': update id ' + id);
        this.setForeignState(id, value);
    }

    _onClientCommand(client, command, value = null) {
        this.log.info(client._id + ': Call command: ' + command);
